...
public class StatusIcon {
	...
	public StatusIcon(@Nullable SettingsStore settings, @Nullable InputMode mode, @Nullable Language language) {
		if (ICONS.isEmpty()) {
			generateIconsCache();
		}
		resourceId = resolveResourcePerMode(settings, mode, language);
	}
	...
	private int resolveResourcePerMode(@Nullable SettingsStore settings, @Nullable InputMode mode, @Nullable Language language) {
		...
		if (InputModeKind.isHiragana(mode)) {
			...
			return getResourceId(getResourceName(mode, language), R.drawable.ic_keyboard);
	...
	@Nullable
	private String getResourceName(@Nullable InputMode mode, @Nullable Language language) {
		...
		int textCase = language.hasUpperCase() ? mode.getTextCase() : InputMode.CASE_UNDEFINED;
		switch (textCase) {
